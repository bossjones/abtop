...
mod tests {
    ...
    #[test]
    fn with_hidden_copilot_drops_copilot_only() {
        let all = MultiCollector::with_hidden(&[]).collectors.len();
        let hidden = MultiCollector::with_hidden(&["CoPiLoT".to_string()])
            .collectors
            .len();
        assert_eq!(hidden, all - 1);
    }
